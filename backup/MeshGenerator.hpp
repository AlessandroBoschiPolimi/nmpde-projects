...
public:
    ...
    CubeGeneratorWithSimplices(std::vector<int> boundary_ids_): boundary_ids(boundary_ids_) {};
    ...
    {
		Triangulation<dim> mesh_serial;
		GridGenerator::subdivided_hyper_cube_with_simplices(mesh_serial, 10, 0.0, 1.0, true);
    ...
    bool OnNeumannBoundary(const int id) const override { return !(std::find(boundary_ids.begin(), boundary_ids.end(), id) != boundary_ids.end()); }
    static constexpr int left = 0,  right = 1, front = 2, back = 3, bottom = 4, top = 5;
};
